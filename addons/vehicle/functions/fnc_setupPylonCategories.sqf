 ...
 *
 * Example:
 * [this] call ghostD_vehicle_fnc_setupPylonCategories;
 *
 * Public: No
 */
...
// Icons
//private _icon  = "iconTank" call EFUNC(systems,getIcon);
...

{
    ...
    if (_vehicle isKindOf _classname) then {
        ...
        } forEach (keys (EGVAR(init,PYLONS) get _classname));
        break;
    };
...
private _vehiclePylon = [QEGVAR(Actions_Vehicle,Pylon_Cat), "Vehicle Loadouts", "", {true}, { call EFUNC(systems,checkStagingZone) }] call ace_interact_menu_fnc_createAction;
