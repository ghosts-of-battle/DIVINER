 ...
 *
 * Example:
 * ["ACE_EarPlugs"] call ghostD_logistics_fnc_checkItemValidity;
 *
 * Public: No
 */
...
private _fn_isValid = {
    ...
    if (_item isKindOf _kindOfItem) then {
        [false,_variable] select (_variable == "varNil")
    } else {
        true
    };
};
...
if (_cfgMagazines == "" && _cfgWeapons == "" && _cfgVehicles == "") exitWith {
    SHOW_WARNING_1("Item","Item '%1' does not exist in CfgMagazines / CfgWeapons / CfgVehicles.",_item);
    false
};
