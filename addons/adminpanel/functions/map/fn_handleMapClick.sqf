...
private _worldPos = _map_ctrl ctrlMapScreenToWorld _clickPos; // convert to world coordinates
...
if (isNull _player) exitWith {["Admin Panel", "No target found!", [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";}; // The next command combinations must have a target to function
...
if (_shift && !_ctrl && !_alt) then { 
    ...
    if (_vehicle == _player || driver _vehicle != _player) then { // if the player is not in a vehicle or isn't the driver tp just them
        ...
        ["Admin Panel", format ["Teleported %1 to grid %2!", name _player, mapGridPosition _safePos], [0.4, 0.702, 0.4, 1]] call EFUNC(notify,notify);
        playSound "3DEN_notificationDefault";
    } else {
        if (_vehicle isKindOf "AIR" && isEngineOn _vehicle) exitWith {["Admin Panel", format ["Engine must be off to teleport aircraft!"], [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";};
        
        // find safe position and teleport player there if it can be found
        _safePos = _worldPos findEmptyPosition [0,30, typeOf _vehicle];
        if (_safePos isEqualTo []) exitWith {["Admin Panel", format ["Unable to find safe teleport location!"], [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";};
    ...
    if (_safePos isNotEqualTo []) then { // if position was found centre map on it
...
if (_alt && !_shift && !_ctrl) then { 
    ...
    if (_target == player) exitWith {["Admin Panel", "No need to teleport to yourself!", [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";}; 

    private _vehicle = vehicle _target;
    private _safePos = [];

    if (objectParent player isEqualTo _vehicle) exitWith {["Admin Panel", "Already in target vehicle!", [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";}; // if the player is already in the vehicle they're teleporting to, cancel
    
    if !(isNull objectParent player) then {moveOut player;}; // if the local player tries to teleport to someone else while in a vehicle, dismount them first
    ...
    ["Admin Panel", format ["Teleported to %1!", name _target], [0.4, 0.702, 0.4, 1]] call EFUNC(notify,notify);
    playSound "3DEN_notificationDefault";
...
if (_shift && _ctrl && !_alt) then { 
    ...
    if (_confirm) then {
        ...
        ["Admin Panel", format ["Teleported %1's group to grid %2!", name _player, mapGridPosition _safePos], [0.4, 0.702, 0.4, 1]] call EFUNC(notify,notify);
        playSound "3DEN_notificationDefault";
...
if (_shift && _alt && !_ctrl) then { 
    ...
    if (_player == player) exitWith {["Admin Panel", "No need to teleport to yourself!", [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";}; 

    private _vehicle = vehicle player;
    private _safePos = [];

    if (objectParent _player isEqualTo _vehicle) exitWith {["Admin Panel", "Target is already in the same vehicle!", [0.831, 0.267, 0.267, 1]] call EFUNC(notify,notify); playSound "addItemFailed";}; // if the player is already in the vehicle they're teleporting to, cancel
    
    if !(isNull objectParent _player) then {moveOut _player;}; // dismount target before bringing them to your position
    ...
    ["Admin Panel", format ["Brought %1 to you!", name _player], [0.4, 0.702, 0.4, 1]] call EFUNC(notify,notify);
    playSound "3DEN_notificationDefault";
};
