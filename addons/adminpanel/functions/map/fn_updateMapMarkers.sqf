...
if (!admp_map_markersActive) then {
...

    {
        ...
        private _data = [_x] call admp_fnc_getMarkerInfo;
    ...
    // WHAT IS BEING TRACKED IS THE TAB'S DECISION, not this function's. It was
    // admp_playerlist_playerArray directly, which is players and only players -
    // see EFUNC(adminpanel,mapTargets) and the strip over the map.
    } forEach ([] call FUNC(mapTargets));
