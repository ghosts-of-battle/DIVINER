...
if (_vehicle == _unit) then {
    ...
    if (driver _vehicle == _unit) then { // if the unit is the driver their name should be used
        private _passengers = ({alive _x} count crew _vehicle)-1; // get number of units in vehicle to display
...
private _markerType = ["mil_triangle_noShadow", "mil_arrow2_noShadow"] select _inVehicle;

private _markerColour = [format ["Color%1", str side _unit], "ColorCivilian"] select (side _unit == sideEnemy);
