...
// --- SUPPORT, over Simplex ---------------------------------------------------
// NO SIMPLEX, NO TILE. The app is not registered without it either - a live tile
// reading NO LINK for a mod that is not installed is a permanent apology, and
// the band is only six wide.
if (EGVAR(patches,usesSimplex)) then {
    ...
    {
        {
            _sssTotal = _sssTotal + 1;
            if ((_x getVariable ["sss_cooldownTimer", 0]) <= 0) then { _sssReady = _sssReady + 1 };
        } forEach ([_unit, _x] call sss_common_fnc_getEntities);
    } forEach (keys (missionNamespace getVariable ["sss_common_services", createHashMap]));

    _out pushBack [
        "support",
        "SUPPORT",
        ["NONE", "ON CALL"] select (_sssTotal > 0),
        str _sssTotal,
        [
            "NOTHING COMMISSIONED",
            format ["%1 READY - PRESS TO TASK", _sssReady]
        ] select (_sssTotal > 0),
        ["stale", "rest"] select (_sssTotal > 0)
    ];
};
...
// built without ghostD_pac has no tile, the same rule as Simplex and PLP.
if (!isNil "ghostD_pac_summary") then {
    private _pac = ghostD_pac_summary;
