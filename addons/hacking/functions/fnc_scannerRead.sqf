...
/* ----------------------------------------------------------------------------
Function: ghostD_hacking_fnc_scannerRead
...
private _jam = 0;
if (!isNil "ghostD_jamming_fnc_jamFactor") then {
    ...
    _jam = ([_pos, 0, "any"] call ghostD_jamming_fnc_jamFactor) param [0, 0];
};
...
private _jamGps = 0;
if (!isNil "ghostD_jamming_fnc_jamFactor") then {
    _jamRadio = ([_pos, 0, "radio"] call ghostD_jamming_fnc_jamFactor) param [0, 0];
    _jamData = ([_pos, 0, "data"] call ghostD_jamming_fnc_jamFactor) param [0, 0];
    _jamGps = ([_pos, 0, "gps"] call ghostD_jamming_fnc_jamFactor) param [0, 0];
};
