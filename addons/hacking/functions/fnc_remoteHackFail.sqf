...
/* ----------------------------------------------------------------------------
Function: ghostD_hacking_fnc_remoteHackFail
...
if (!isNil "ghostD_jamming_fnc_spawnTempZone" && {random 100 < _jamChance}) then {
    private _r = (RH_JAM_MIN_DEF + random (RH_JAM_MAX_DEF - RH_JAM_MIN_DEF)) min RH_JAM_MAX_CAP;
    private _zone = [_pos, _r, RH_JAM_DURATION_DEF] call ghostD_jamming_fnc_spawnTempZone;
