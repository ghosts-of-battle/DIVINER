...
if (isServer) then {
    ...
    // A WITNESSED POP HACK, ANNOUNCED AND NOT SCORED. This used to raise the
    // local settlement's hostility through the adapter, which read and wrote
    // ALiVE's civilian model. There is no such model now, so nobody keeps
    // score - but the event still fires and still logs, so a mission that
    // wants to answer a hack somebody saw has a place to listen.
    [QGVAR(popSeen), {
...
if (isServer) then {
    ...
    // THE INTEL TALLY NO LONGER PERSISTS. Banked deposits, the hint tier and
    // the list of sites already named were read out of ALiVE's own save on its
    // ready event and written back as they changed, so a campaign that reloaded
    // its war did not reload it with the intel economy reset to zero. There is
    // no save to read now. A restarted mission starts the tally at nothing,
    // which is what every other counter in this mod already does.
